...
func main() {
	...
	b.Handle(tele.OnText, func(c tele.Context) error {
		...
		if userActiveLink[u.UID] == true {
			...
			// if !ghRepo.ValidLinkPublic(ghLink) {
			// 	if u.UserLanguage == "en" {
			// 		return c.Send("Oops, I couldn't connect. The repository may not exist \n")
			// 	} else {
			// 		return c.Send("Упс, я не смог подключиться. Возможно репозитория не существует \n")
			// 	}
			// }

			// if u.UserLanguage == "en" {
			// 	err = ghRepo.CloneRep(ghLink, u.UID)
			// 	if err != nil {
			// 		return c.Send("ERORR!")
			// 	}
			// 	return c.Send("I accepted the repository, I’m starting the analysis:\n" + ghLink)
			// } else {
			// 	return c.Send("Принял репозиторий, начинаю анализ: \n" + ghLink)
			// }
		}
		return nil
